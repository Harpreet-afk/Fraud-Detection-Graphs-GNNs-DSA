...
-----------------------------------------
Reads the original raw data with string node IDs (C..., M...), builds a
string -> integer node mapping, and outputs numeric versions of edges.csv,
labels.csv, and raw_features.csv for the C++ module and GNN pipeline.

The C++ module is responsible for graph construction and structural feature
computation (degree, PageRank, clustering coefficient).
...
Output files (in ../data/):
    - edges.csv        : numeric edge list (source, target) — overwrites input
    - raw_features.csv : transactional features with numeric node IDs — overwrites input
    - labels.csv       : numeric labels (node, label) — overwrites input
...
OUTPUT_RAW_FEATURES = os.path.join(DATA_DIR, "raw_features.csv")
...
# --------------------------------------------------------------------------
# Step 2: Convert edges to numeric
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# Step 3: Convert raw features to numeric node IDs
# --------------------------------------------------------------------------
def convert_raw_features(raw_path, node_mapping, output_path):
    ...
    print(f"[OK] Wrote numeric raw_features.csv ({len(df)} rows, columns = {list(df.columns)})")
...
def main():
    print("=" * 60)
    print("  Fraud Detection - Preprocessing (ID Mapping)")
    print("=" * 60)
    ...
    # 3. Convert edges to numeric
    print("\n--- Converting edges ---")
    convert_edges(edges_df, node_mapping, OUTPUT_EDGES)

    # 4. Convert raw features to numeric node IDs
    print("\n--- Converting raw features ---")
    convert_raw_features(RAW_FEATURES, node_mapping, OUTPUT_RAW_FEATURES)
    ...
    print("  Next step: run the C++ module to compute features")
    print("=" * 60)
